...
// Update the "Total Sum" row in the updateTotalSum method
  ...
  void initialiseStates() {
    ...
    if (parquetAndLaminateCalculationControllers.text != "")
      calculationQuantity =
          double.parse(parquetAndLaminateCalculationControllers.text);
  }
  ...
  void _updateLaborHours() {
    ...
    for (int i = 0; i < parquetAndLaminate.length; i++) {
      if (parquetAndLaminate[i].name == widget.name) {
        setState(() {
          for (int j = 0; j < parquetAndLaminate[i].laborHours1.length; j++) {
            widget.laborHours1[j] = parquetAndLaminate[i].laborHours1[j];
          }
        });
        return;
      }
