...
class LitSupportSystemItemScreen extends StatefulWidget {
  String name;
  ...
  @override
  State<LitSupportSystemItemScreen> createState() =>
      _LitSupportSystemItemScreenState();
}

TextEditingController litSupportSystemCalculationControllers =
...
// Update the "Total Sum" row in the updateTotalSum method
  ...
  void initialiseStates() {
    ...
    if (litSupportSystemCalculationControllers.text != "")
      calculationQuantity =
          double.parse(litSupportSystemCalculationControllers.text);
  }
  ...
  void _updateLaborHours() {
    ...
    for (int i = 0; i < litSupportSystem.length; i++) {
      if (litSupportSystem[i].name == widget.name) {
        setState(() {
          for (int j = 0; j < litSupportSystem[i].laborHours1.length; j++) {
            widget.laborHours1[j] = litSupportSystem[i].laborHours1[j];
          }
        });
        return;
      }
...
// Add the "Total Sum" row to the rows list
  ...
  Future<String?> openDialog() => showDialog<String>(
        context: context,
        builder: (context) => AlertDialog(
          title: Text("Suteikite failui pavadinimą"),
          content: TextField(
