...
class LitInteriorWallItemsScreen extends StatefulWidget {
  String name;
...
// Update the "Total Sum" row in the updateTotalSum method
  ...
  void _resetLaborHours() {
    ...
    for (int i = 0; i < litInnerWallData.length; i++) {
      if (litInnerWallData[i].name == widget.name) {
        setState(() {
          for (int j = 0; j < litInnerWallData[i].laborHours1.length; j++) {
            widget.laborHours1[j] = litInnerWallData[i].laborHours1[j];
          }
        });
        return;
      }
