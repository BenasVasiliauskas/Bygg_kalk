...
class LitScaffoldingItemScreen extends StatefulWidget {
  String name;
  ...
  @override
  State<LitScaffoldingItemScreen> createState() =>
      _LitScaffoldingItemScreenState();
}

TextEditingController litScaffoldingCalculationControllers =
...
// Update the "Total Sum" row in the updateTotalSum method
  ...
  void initialiseStates() {
    ...
    if (litScaffoldingCalculationControllers.text != "")
      calculationQuantity =
          double.parse(litScaffoldingCalculationControllers.text);
  }
  ...
  void _updateLaborHours() {
    ...
    for (int i = 0; i < litScaffoldingData.length; i++) {
      if (litScaffoldingData[i].name == widget.name) {
        setState(() {
          for (int j = 0; j < litScaffoldingData[i].laborHours1.length; j++) {
            widget.laborHours1[j] = litScaffoldingData[i].laborHours1[j];
          }
        });
        return;
      }
                        ...
                        (value) {
                          for (int i = 0; i < value.length; i++) {
                            ScaffoldingModel scaffoldingModel =
                                ScaffoldingModel.fromJson(value[i]);
                            if (scaffoldingModel.name == widget.name) {
                              setState(() {
                                widget.description =
                                    scaffoldingModel.description;
                                widget.unit = scaffoldingModel.unit;
                                widget.quantity = scaffoldingModel.quantity;
                                widget.laborHours1 =
                                    scaffoldingModel.laborHours1;
                                widget.laborHours2 =
                                    scaffoldingModel.laborHours2;
                                widget.laborCost = scaffoldingModel.laborCost;
                                widget.material1 = scaffoldingModel.material;
                                widget.material2 = scaffoldingModel.materials;
                                widget.totalPrice = scaffoldingModel.totalPrice;
                                setInitialValues();
