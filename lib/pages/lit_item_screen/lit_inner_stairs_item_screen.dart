...
class LitInnerStairsItemScreen extends StatefulWidget {
  String name;
  ...
  @override
  State<LitInnerStairsItemScreen> createState() =>
      _LitInnerStairsItemScreenState();
}

//
TextEditingController litInnerStairsCalculationControllers =
...
// Update the "Total Sum" row in the updateTotalSum method
  ...
  void initialiseStates() {
    ...
    if (litInnerStairsCalculationControllers.text != "")
      calculationQuantity =
          double.parse(litInnerStairsCalculationControllers.text);
  }
  ...
  void _updateLaborHours() {
    ...
    for (int i = 0; i < litInnerStairsData.length; i++) {
      if (litInnerStairsData[i].name == widget.name) {
        setState(() {
          for (int j = 0; j < litInnerStairsData[i].laborHours1.length; j++) {
            widget.laborHours1[j] = litInnerStairsData[i].laborHours1[j];
          }
        });
        return;
      }
