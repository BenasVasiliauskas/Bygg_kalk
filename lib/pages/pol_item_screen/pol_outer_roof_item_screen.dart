...
class PolOuterRoofItemScreen extends StatefulWidget {
  String name;
...
// Update the "Total Sum" row in the updateTotalSum method
  ...
  void _resetLaborHours() {
    ...
    for (int i = 0; i < polOuterRoofData.length; i++) {
      if (polOuterRoofData[i].name == widget.name) {
        setState(() {
          for (int j = 0; j < polOuterRoofData[i].laborHours1.length; j++) {
            widget.laborHours1[j] = polOuterRoofData[i].laborHours1[j];
          }
        });
        return;
      }
      ...
      rows.add(
        DataRow(
          cells: [
            ...
            dataCellDisplay(widget.unit, i, 30, true, optionalPadding: 12),
