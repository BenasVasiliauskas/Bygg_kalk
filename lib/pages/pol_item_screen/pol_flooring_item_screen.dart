...
class PolFlooringItemScreen extends StatefulWidget {
  String name;
...
// Update the "Total Sum" row in the updateTotalSum method
  ...
  void _resetLaborHours() {
    ...
    for (int i = 0; i < polFlooringData.length; i++) {
      if (polFlooringData[i].name == widget.name) {
        setState(() {
          for (int j = 0; j < polFlooringData[i].laborHours1.length; j++) {
            widget.laborHours1[j] = polFlooringData[i].laborHours1[j];
          }
        });
        return;
      }
