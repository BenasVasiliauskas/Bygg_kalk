...
// Update the "Total Sum" row in the updateTotalSum method
  ...
  void _resetLaborHours() {
    ...
    for (int i = 0; i < polHullRoofingData.length; i++) {
      if (polHullRoofingData[i].name == widget.name) {
        setState(() {
          for (int j = 0; j < polHullRoofingData[i].laborHours1.length; j++) {
            widget.laborHours1[j] = polHullRoofingData[i].laborHours1[j];
          }
        });
        return;
      }
