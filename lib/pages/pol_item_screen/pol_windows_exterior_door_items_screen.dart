...
class PolWindowsExteriorDoorItemsScreen extends StatefulWidget {
  String name;
  ...
  @override
  _PolWindowsExteriorDoorItemsScreenState createState() =>
      _PolWindowsExteriorDoorItemsScreenState();
}

//
TextEditingController polWindowsExteriorDoorsCalculationControllers =
...
// Update the "Total Sum" row in the updateTotalSum method
  ...
  void initialiseStates() {
    ...
    if (polWindowsExteriorDoorsCalculationControllers.text != "")
      calculationQuantity =
          double.parse(polWindowsExteriorDoorsCalculationControllers.text);
  }
  ...
  void _updateLaborHours() {
    ...
    for (int i = 0; i < polWindowsExteriorDoors.length; i++) {
      if (polWindowsExteriorDoors[i].name == widget.name) {
        setState(() {
          for (int j = 0;
              j < polWindowsExteriorDoors[i].laborHours1.length;
              j++) {
            widget.laborHours1[j] = polWindowsExteriorDoors[i].laborHours1[j];
          }
        });
        return;
      }
