...
class PolInnerStairsItemScreen extends StatefulWidget {
  String name;
  ...
  @override
  State<PolInnerStairsItemScreen> createState() =>
      _PolInnerStairsItemScreenState();
}

//
TextEditingController polInnerStairsCalculationControllers =
...
// Update the "Total Sum" row in the updateTotalSum method
  ...
  void initialiseStates() {
    ...
    if (polInnerStairsCalculationControllers.text != "")
      calculationQuantity =
          double.parse(polInnerStairsCalculationControllers.text);
  }
  ...
  void _updateLaborHours() {
    ...
    for (int i = 0; i < polInnerStairsData.length; i++) {
      if (polInnerStairsData[i].name == widget.name) {
        setState(() {
          for (int j = 0; j < polInnerStairsData[i].laborHours1.length; j++) {
            widget.laborHours1[j] = polInnerStairsData[i].laborHours1[j];
          }
        });
        return;
      }
